...
def classic(target_func: callable(list[float]),
    ...
    while (iteration < max_iter) & (target_func_curr_value > tf_precision):
        ...
        # Запоминаем лучше значения коэффициентов на случай вылета за отсечку по итерациям, т.к. значение
        # целевой функции может скакать в процессе расчета и последние значения в случае вылета могут не быть лучшими
        if target_func_curr_value < target_func_min_value:
            target_func_min_value = target_func_curr_value
            best_values = values
...
def gauss_seidel(target_func, values: list[float],
    ...
    while (inner_iteration < max_iter) & (target_func_curr_value > tf_precision):
        ...
        log.debug("({}); tf = {}; tf_grad={}".format(iteration, target_func_curr_value,
                                                     helpers.gradient(target_func, values)))
        for i in range(len(values) - 1, -1, -1):
            curr_step = step
            tf_partial_derivative = helpers.partial_derivative(target_func, values, i)
            ...
            while (inner_iteration < max_iter) & \
                ...
                tf_partial_derivative_next = helpers.partial_derivative(target_func, values, i)
                derivative_will_step_over_zero = ((tf_partial_derivative_next > 0) & (
                        tf_partial_derivative < 0)) | ((tf_partial_derivative_next < 0) & (tf_partial_derivative > 0))
